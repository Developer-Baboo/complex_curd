    ...
    <div class="container">
        <form action="{{ route('update', ['id' => $record->id]) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-check">
                <input class="form-check-input" name="skill[]" type="checkbox" value="java" id="flexCheckJava"
                    {{ in_array('java', explode(',', $record->skill)) ? 'checked' : '' }}>
                <label class="form-check-label" for="flexCheckDefault">
                    Java
                </label>
            </div>
            <div class="form-check">
                <input class="form-check-input" name="skill[]" type="checkbox" value="python" id="flexCheckPython"
                    {{ in_array('python', explode(',', $record->skill)) ? 'checked' : '' }}>
                <label class="form-check-label" for="flexCheckChecked">
                    Python
                </label>
            </div>
            <div class="form-check">
                <input class="form-check-input" name="skill[]" type="checkbox" value="c++" id="flexCheckC++"
                    {{ in_array('c++', explode(',', $record->skill)) ? 'checked' : '' }}>
                <label class="form-check-label" for="flexCheckChecked">
                    C++
                </label>
            </div>
